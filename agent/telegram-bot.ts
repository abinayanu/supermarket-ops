...
import { handleWithVercelAgent } from './llm-agent';
...
async function main(): Promise<void> {
  ...
  bot.use(async (ctx, next) => {
    ...
    if (chatId !== ownerChatId) {
      console.log(`Blocked message from unauthorized chat ID: ${chatId ?? 'unknown'}`);
  ...
  bot.on('text', async (ctx) => {
    ...
    try {
      ...
      if (normalizedText.includes('pdf') || normalizedText.includes('invoice')) {
        ...
        if (billId) {
          ...
          await ctx.reply('No finalized bill found. Please finalize a bill first.');
          return;
        }
      }


      // Check if message is a UUID (product selection)
      const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
      if (uuidRegex.test(text.trim())) {
        // Forward UUID to LLM agent as product selection
        const reply = await handleWithVercelAgent(chatId, `SELECT_PRODUCT:${text.trim()}`);
        await ctx.reply(reply);
        return;
      }


      const reply = await handleWithVercelAgent(chatId, text);
      await ctx.reply(reply);
    } catch (error: any) {
